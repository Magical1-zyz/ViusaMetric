...
namespace Core {
    ...
    void PBRRenderer::SetupFBO() {
        ...
        // Normal (Output 1) - RGB16F High Precision
    ...
    void PBRRenderer::InitIBL(const std::string& hdrPath) {
    ...
    void PBRRenderer::RenderModel(Model& model, bool isRefModel) {
        ...
        if (isRefModel) {
            pbrShader->setBool("hasNormalMap", true);
            pbrShader->setBool("hasMRMap", true);
        } else {
            pbrShader->setBool("hasNormalMap", false);
            pbrShader->setBool("hasMRMap", false);
            pbrShader->setFloat("u_RoughnessDefault", 1.0f);
